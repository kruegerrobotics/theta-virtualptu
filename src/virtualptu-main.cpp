...
int main(int argc, char *argv[]) {
  ...
  while (thetaCtrl.isSleeping()) {
    if (loopCount <= maxRetries) {
      std::cout << "theta is in sleep mode - initiate wake up - attempt: "
  ...
  // this is dangerous when the device switches modes the USB connetion and id
  ...
  GstCaps *caps = gst_caps_new_simple(
      "video/x-h264", "framerate", GST_TYPE_FRACTION, 30000, 1001,
      "stream-format", G_TYPE_STRING, "byte-stream", "profile", G_TYPE_STRING,
...
void joystickThreadBody(bool &abortFlag, std::string joystickFd,
  ...
  while (!abortFlag) {
    ...
    if (rv == -1) {
      ...
      // std::cout << "timeout no: " << timeOutCount << std::endl; /* a timeout
      // occured */
      timeOutCount++;
    } else {
