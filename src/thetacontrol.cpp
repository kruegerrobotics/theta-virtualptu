...
int ThetaControl::setProperty(uint32_t propertyCode, uint16_t value)
{
    ...
    int res = ptp_transaction(&ptpParams, &ptp, PTP_DP_SENDDATA, sizeof(value), &dpv);
    if (res != PTP_RC_OK)
    {
            ...
            if (dev->config)
            {
                     ...
                     USB_CLASS_PTP))
                {
                    if (dev->descriptor.bDeviceClass != USB_CLASS_HUB)
                    {
                        ...
                        //if we wind the theta V - expection to have "RICOH THETA V" as deviceinfo.Model
...
int ThetaControl::disableSleepMode()
{
    ...
    const uint32_t stillCaptureMode = 0x5013;
    int res = checkProperty(stillCaptureMode);
    std::cout << "stillCaptureMode: " << res << std::endl;

    return -1;
}

bool ThetaControl::isInStreamingMode()
